...
if __name__ == '__main__':
    evaluator = EvalUtil()
    gt_joint = torch.randn((64, 21, 3)) * 10
    pred_joint = torch.randn((64, 21, 3)) * 10
    print(gt_joint.shape)
    print(gt_joint[0])
    total_test_auc = 0
    for i in range(64):
        gt_joint_item = gt_joint[i]
        pred_joint_item = pred_joint[i]
    ...
    print("一个batch整体auc {}".format(total_test_auc / 64))
    # 只针对一个样本进行计算
    # evaluator.feed(gt_joint, pred_joint)
    # result = evaluator.get_measures(0, 50, 20)
    # print(result)
    # _1, _2, _3, auc_2d, pck_curve_2d, _ = result
    # print(type(auc_2d))
    # print(pck_curve_2d)
